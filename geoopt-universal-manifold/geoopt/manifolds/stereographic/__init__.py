...
class Stereographic(Manifold):
  ...
  def __init__(
    ...
    self.initial_sign = torch.nn.Parameter(torch.sign(K),requires_grad=False)
  ...
  def get_K(self) -> torch.Tensor:
    ...
    """
    if self.keep_sign_fixed:
      return self.initial_sign * (self.min_abs_K + self._softplus(self.trainable_K))
    else:
  ...
  def geodesic_unit(self, t: torch.Tensor, x: torch.Tensor, u: torch.Tensor, *, 
  ...
  def gyration(self, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor, *, 
               dim=-1) -> torch.Tensor:
  ...
  def mobius_fn_apply(self, fn: callable, x: torch.Tensor, *args, dim=-1, 
                      project=True,**kwargs) -> torch.Tensor:
  ...
  # this does not yet work with scaling
  @__scaling__(ScalingInfo.NotCompatible)
  def mobius_fn_apply_chain(self, x: torch.Tensor, *fns: callable, project=True, 
                            dim=-1) -> torch.Tensor:
  ...
  def random_normal(self, *size, mean=0, std=1, dtype=None, device=None
    ...
    tens = torch.randn(*size,device=self.c.device,dtype=self.c.dtype) \
            * std + mean
