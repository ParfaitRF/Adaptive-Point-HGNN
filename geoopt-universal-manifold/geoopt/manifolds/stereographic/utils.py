...
def setup_plot(manifold, lo=None, width=7, height=7, grid_line_width=0.3, with_background=True):
  ...
  if K > 0:
    circle_border = plt.Circle((0, 0), R, fill=False, color=COLORS.GREY,linewidth=2.0)
...
def get_interpolation_Ks(num=200):
  ...
  Ks  = (x**3).tolist()
  Ks  = [K for K in Ks if abs(K) > 1e-3]
  return Ks
...
def save_img_sequence_as_boomerang_gif(imgs, out_filename, fps=24,as_boomerang=False):
