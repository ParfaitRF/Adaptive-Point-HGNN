...
def show_me():
  ...
  def orthogonal_proj(x,a):
    """
    Orthogonal projection of :math:`\mathbf{x}` onto the geodesic through the
    origin :math:`\mathbf{0}` and :math:`\mathbf{a}`
    :param x: the point :math:`\mathbf{x}` to project
    :param a: the point :math:`\mathbf{a}` on the geodesic
    :return: :math:`\mathbf{x}_p`, the orthogonal projection of $x$
    """
