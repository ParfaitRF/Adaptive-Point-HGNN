...
def boxes_3d_to_corners(boxes_3d):
    """ Tranlsates bounding boxes from origin to place in IR`3 
    """
    all_corners = []
    for box_3d in boxes_3d:
        ...
        cam_points_xyz = r_corners+np.array([x3d, y3d, z3d]) # translates cube from origin to coords.
        all_corners.append(cam_points_xyz)
    return np.array(all_corners)

def overlapped_boxes_3d(single_box, box_list):
    x_max, y_max, z_max = np.max(single_box, axis=0)
    x_min, y_min, z_min = np.min(single_box, axis=0)
    overlap = np.zeros(len(box_list))
    for i, box in enumerate(box_list):
        xi_max, yi_max, zi_max = np.max(box, axis=0)
        xi_min, yi_min, zi_min = np.min(box, axis=0)
        if x_max < xi_min or x_min > xi_max:
            overlap[i] = 0
            continue
        if y_max < yi_min or y_min > yi_max:
            overlap[i] = 0
            continue
        if z_max < zi_min or z_min > zi_max:
            overlap[i] = 0
            continue
        # define view plane
        x_draw_min = min(x_min, x_min)
        x_draw_max = max(x_max, x_max)
        z_draw_min = min(z_min, z_min)
        z_draw_max = max(z_max, z_max)
        ...
        y_overlap = min(y_max, y0_max) - max(y_min, y0_min)
        intersection = y_overlap * xz_overlap
        union = (yi_max-yi_min) * area2 + (y_max-y_min) * area1 - overlap
        overlap[i] = np.float32(intersection) / union
    return overlap
